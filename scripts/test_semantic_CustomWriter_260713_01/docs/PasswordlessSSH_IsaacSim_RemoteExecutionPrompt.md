...
23. 不要覆盖 authorized_keys；增加新公钥时只能追加，并避免重复条目。
24. 远端 /root/.ssh 权限应为 700，authorized_keys 权限应为 600。
25. 如果 BatchMode 连接失败，应先检查 SSH 配置、密钥路径和文件权限，再向用户报告；不要退回密码认证。
```
...
同时确认 `authorized_keys` 中存在对应 `.pub` 公钥。不要复制或查看私钥。
