...
先复制 `configs/capture_motion_camera02.json` 为
`configs/capture_static_smoke.json`，保留全部字段并修改：
...
```json
{
  ...
  "output": "../output/smoke/static_001"
}
```
...
```bash
python -m json.tool output/smoke/static_001/run_config.json | less
find output/smoke/static_001 -maxdepth 2 -type f | sort
...
复制完整样本为 `configs/capture_motion_smoke.json`，保留全部字段并修改：
...
```json
{
  ...
  "height": 360,
  "warmup_render_frames": 4,
  "output": "../output/smoke/motion_001"
}
```
...
```bash
head -n 3 output/smoke/motion_001/motion_state.jsonl
```
