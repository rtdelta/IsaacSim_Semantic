...
入口 `main()` 位于 `simulation_orchestrator.py:361`。在创建 Isaac Sim 前，它先完成：
...
入口用 `parse_known_args()` 而不是 `parse_args()`。项目认识的参数交给业务代码，剩余参数重新放回 `sys.argv`，供 Kit 自己消费。这避免了项目 CLI 与 Kit CLI 相互冲突。
...
注意：`--no-strict-stage` 只放宽通用预检中的部分错误。缺少场景/映射/相机/语义仍会阻断；启用运动时，Articulation 报告也始终必须通过。
