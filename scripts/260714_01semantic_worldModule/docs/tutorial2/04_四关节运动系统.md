...
Loop 还要求最后一帧四个角度与第一帧完全闭合。当前默认 CSV 满足这一条件，因此可以安全传 `--trajectory-mode loop`。
...
默认相机位于驾驶室 Prim 下。`UsdGeom.XformCache.GetLocalToWorldTransform()` 会把父级变换组合进去。因此 cab 旋转时，相机世界矩阵也应变化。
...
这是一种端到端检查：即使关节数字在变，如果 USD 连接错误、变换没有传递，数据集仍会验收失败。
