...
> 2026-07-23 更新：当前采集入口只接受 `--config <json>`。本文中的关节控制设计仍有效，
> 原命令行字段现已迁移为业务配置 JSON 的下划线字段。
...
```text
解析业务启动配置、渲染配置、关节配置和 Recorder sidecar
  -> 打开并等待 USD 完成组合
  -> 通用 Stage preflight
...
Articulation 专项错误不会被业务配置中的 `strict_stage=false` 绕过，因为直接控制在合同不完整时没有
安全、可解释的降级路径。
...
`trajectory_mode` 的样本值为 `hold`。运行清单中的运动合同包括：
...
若确实要循环，必须保证 CSV 最后一行的四个角度与第一行相同，然后在完整业务配置中设置：

```json
{
  "trajectory_mode": "loop"
}
```
