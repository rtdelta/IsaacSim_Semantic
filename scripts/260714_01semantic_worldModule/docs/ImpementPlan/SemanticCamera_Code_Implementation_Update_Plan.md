...
本文档给出 `260714_01semantic_worldModule` 的代码级修正方案，目标是在保留驾驶室语义相机 `/root/Xform/operator_cab_mesh/Camera_01`、固定步长物理、RGB 与语义联合输出等现有能力的前提下，解决当前采集管线中已经确认或高度怀疑的以下问题：
...
- Camera_01 仍然是生产语义相机；Camera_03 只用于构造 GUI/脚本渲染基准。
...
- 不通过把生产相机从 Camera_01 换成 Camera_03 来掩盖画质问题。
...
| 文件 | 变更类型 | 主要内容 |
|---|---|---|
| `simulation_orchestrator.py` | 重点修改 | 初始化顺序、CLI、render profile、preflight、帧调度、运行取证 |
...
| `run_capture_remote.sh` | 小幅扩展 | 显式 profile、命令行快照、运行环境清单；保留隔离 HOME |
...
```text
--render-profile <json path>
--capture-mode static|motion
--capture-initial-frame / --no-capture-initial-frame
--pre-roll-steps <int>
--strict-stage / --no-strict-stage
--frame-sync blocking|queued
--quality-reference <optional path>
```

现有参数调整：

- `--rt-subframes` 可保留为 profile 的临时覆盖项，但最终解析值必须写入 resolved profile。
- 默认 `rt_subframes` 从 4 调整到 16，只在质量基线通过后再决定是否下调。
- `--warmup` 明确改名或解释为 `--warmup-render-frames`，避免继续把普通 `app.update()` 次数等同于有效渲染预热。
- `--headless` 继续保留，但不能再隐式决定画质参数。
...
- 启动前打印项目路径和实际命令参数，但不打印敏感信息；
- 将选用的 render profile 绝对路径传给 Python；
- 保存 Isaac Sim 启动入口和关键环境变量名称；
- 保持 `exec`，确保退出码来自 Python 主程序；
- 不在 shell 中暗中覆盖 renderer、DLSS 或 subframes。

默认 README 命令中的 `--rt-subframes 4` 应在质量基线完成后更新为 profile 驱动；第一版建议使用 16。
